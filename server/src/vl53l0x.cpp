...
#define startTimeout() (timeout_start_ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count())

// Check if timeout is enabled (set to nonzero value) and has expired
#define checkTimeoutExpired() (io_timeout > 0 && (uint16_t(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count() - timeout_start_ms) > io_timeout))
...
uint8_t VL53L0X::readReg(uint8_t reg)
{
    ...
    uint8_t value = 0;
    bus->writeByte(address, reg);
    bus->readBytes(address, &value, 1);
    return value;
}
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
    ...
    uint16_t value;
    bus->writeByte(address, reg);
    bus->readBytes(address, data, 2);
    value = uint16_t(data[0]) << 8 | data[1];
    return value;

    // uint16_t value = 0;
    // bus->writeByte(address, reg);
    // bus->readBytes(address, reinterpret_cast<uint8_t*>(&value), 1);
    // return value;
}
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
    ...
    bus->writeByte(address, reg);
    bus->readBytes(address, data, 2);
    value = uint32_t(data[0]) << 24 | uint32_t(data[1]) << 16 | uint16_t(data[2]) << 8 | data[3];
    return value;
}
...
void VL53L0X::writeMulti(uint8_t reg, uint8_t const *src, uint8_t count)
{
    ...
    bus->writeBytes(address, data, count + 1);
    delete[] data;
}
...
void VL53L0X::readMulti(uint8_t reg, uint8_t *dst, uint8_t count)
{
    ...
    // }
    bus->writeByte(address, reg);
    bus->readBytes(address, dst, count);
}
...
void VL53L0X::poll()
{
    m_distance = readRangeSingleMillimeters() / 1000.f;
}
